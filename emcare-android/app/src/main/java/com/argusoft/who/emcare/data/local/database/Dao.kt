...
interface Dao {
    ...
    @Query("SELECT * from consultationflowitem where isActive=1 AND consultationDate in (SELECT MAX(consultationDate) from consultationflowitem group by encounterId) ORDER BY consultationDate DESC LIMIT :limit OFFSET :offset")
    suspend fun getAllLatestActiveConsultationsPaginated(limit: Int, offset: Int): List<ConsultationFlowItem>?
